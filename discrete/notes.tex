...
% Make separate theorem environments
% Make separate corollary environments

\newcommand\fn[3]{#1 \from #2 \mapsto #3}

% Automatically put sections on new pages if there is not enough space
...
\begin{definition}[Element Argument]\label{def:element-argument}
...
% Corollary
\begin{definition}[The Empty Set is a Subset of Every Set]\label{def:empty-set-thm}
...
\begin{definition}[Empty Set is Unique]\label{def:empty-set-unique}
